my_dir <- "C:/Users/sudesh yadav/OneDrive - IIT Kanpur/Desktop/Linear Regression anova/Lab7"
setwd(my_dir)
### question 1 

library(readxl)
Webster.Data<- read_excel("Webster_Data.xlsx")
x0<-c(rep(1,12))
...
EVec_ratio_eg<-matrix(,nrow=6,ncol=6)
for (i in 1:6){
  for (j in 1:6){
...
EVec_ratio_eg1<-matrix(,nrow=6,ncol=6)
for (j in 1:6){
...
# R code for ridge and Principle component Regressor 
R code for Ridge Estimator and Principle Component Regressor
Acetylene_Data<-read_excel('Acetylene_Data_MCLT.xlsx')
